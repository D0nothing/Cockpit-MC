...
const defaultBudget = {
  ...
  maxConcurrency: simulatorCapacity,
};
...
export function listSessions(prisma: PrismaClient, projectId: string) {
  ...
  return prisma.workSession.findMany({
    ...
    take: 100,
    include: {
      macroTasks: { orderBy: { version: "desc" }, take: 1 },
...
export async function createSession(prisma: PrismaClient, body: unknown) {
  ...
  const session = await prisma.workSession.create({
    data: { projectId, objective, riskLevel, createdBy, idempotencyKey },
  });
...
export async function planSession(
  ...
  if (!session) throw new HttpError(404, "Session not found");
  if (session.macroTasks[0]?.graph)
    return getSession(prisma, sessionId, projectId);
  if (!["created", "planning"].includes(session.state))
    throw new HttpError(409, `Session cannot be planned from ${session.state}`);
  ...
  await prisma.$transaction(
    async (tx) => {
      const ticketCounter = await tx.project.update({
        where: { id: projectId },
        data: {
          nextTicketNumber: { increment: plan.requestPlan.tickets.length },
        },
        select: { nextTicketNumber: true },
      });
      const firstTicketNumber =
        ticketCounter.nextTicketNumber - plan.requestPlan.tickets.length;
      ...
      const epicIds = new Map(
        plan.requestPlan.epics.map((epic) => [
          epic.epicKey,
          `epic-${sessionId}-${epic.epicKey}`,
        ]),
      );
      const resolveEpicId = (epicKey: string) => {
        const epicId = epicIds.get(epicKey);
        if (!epicId)
          throw new HttpError(500, `Planned epic ${epicKey} is missing`);
        return epicId;
      };
      await tx.epic.createMany({
        data: plan.requestPlan.epics.map((epic, sequence) => ({
          id: resolveEpicId(epic.epicKey),
          projectId,
          sessionId,
          key: epic.epicKey,
          title: epic.title,
          objective: epic.objective,
          expectedOutcome: epic.expectedOutcome,
          acceptanceCriteria: epic.acceptanceCriteria,
          sequence: sequence + 1,
        })),
      });
      const persistedTickets = plan.requestPlan.tickets.map(
        (ticket, index) => ({
      ...
      await tx.ticket.createMany({
        data: persistedTickets.map(({ id, externalId, ticket }) => ({
          id,
          externalId,
          title: ticket.title,
          ...
          riskLevel: plan.macroTask.riskLevel,
          projectId,
          epicId: resolveEpicId(ticket.epicKey),
      ...
      await tx.specification.createMany({
        data: persistedTickets.map(({ id, ticket }) => ({
          ticketId: id,
          content: plannedSpecification(
            ticket.title,
            ticket.description,
            ticket.acceptanceCriteria,
            ticket.definitionOfDone,
          ),
  ...
  await recordAudit(prisma, {
    projectId,
    actorId,
    action: "session.planned",
    targetType: "session",
    targetId: sessionId,
    metadata: {
      macroTaskId: plan.macroTask.macroTaskId,
      graphId: plan.graph.graphId,
      epics: plan.requestPlan.epics.length,
      tickets: plan.requestPlan.tickets.length,
      requiredApprovals: plan.macroTask.requiredApprovals,
    },
  });
  return getSession(prisma, sessionId, projectId);
...
export function listBacklog(prisma: PrismaClient, projectId: string) {
  ...
  return prisma.epic.findMany({
    ...
    include: {
      session: {
        select: { id: true, objective: true, state: true, riskLevel: true },
      },
      tickets: {
        orderBy: { externalId: "asc" },
        include: { specification: true, workflow: true },
      },
    },
  });
}
...
export async function startRun(
  ...
  if (existing) {
    if (existing.sessionId !== sessionId)
      throw new HttpError(
        409,
        "Idempotency key already used with another session",
      );
  ...
  const session = await prisma.workSession.findFirst({
    ...
    include: {
      macroTasks: {
        orderBy: { version: "desc" },
        take: 1,
        include: { graph: true, approvalRequest: true },
      },
      plannedTickets: { select: { id: true, plannerKey: true } },
    },
  });
  const macroTask = session?.macroTasks[0];
  if (!session || !macroTask?.graph)
    throw new HttpError(409, "A planned session is required");
  if (session.state !== "ready")
    throw new HttpError(409, `Session cannot start from ${session.state}`);
  if (
    macroTask.requiredApprovals > 0 &&
    macroTask.approvalRequest?.status !== "approved"
  )
    throw new HttpError(403, "Required human approval is missing");
  const graph = parsePersistedGraph(
    projectId,
    sessionId,
    macroTask.id,
    macroTask.version,
    macroTask.graph.id,
    macroTask.graph.nodes,
  );
  const ticketIds = new Map(
    session.plannedTickets.flatMap((ticket) =>
      ticket.plannerKey ? [[ticket.plannerKey, ticket.id] as const] : [],
    ),
  );
  if (graph.nodes.some((node) => !ticketIds.has(node.taskId)))
    throw new HttpError(409, "Every run task must reference a planned ticket");
  ...
  const run = await prisma.$transaction(async (tx) => {
    ...
    await tx.runEvent.create({
      data: eventData(
        ...
        "human",
        actorId,
        { macroTaskId: macroTask.id, graphId: graph.graphId },
      ),
    });
    await tx.workSession.update({
      where: { id: sessionId },
...
export async function getRun(
  ...
  const run = await prisma.executionRun.findFirst({
    ...
    include: {
      ...
      session: {
        select: { id: true, objective: true, state: true, version: true },
      },
      macroTask: true,
      graph: true,
      tasks: {
...
export async function commandRun(
  ...
  if (existing) {
    if (existing.runId !== runId || existing.type !== type)
      throw new HttpError(
        409,
        "Idempotency key already used with another command",
      );
    return existing;
  }
  const run = await prisma.executionRun.findFirst({
  ...
  if (!nextState) {
    await recordAudit(prisma, {
      projectId,
      actorId,
      action: "run.command_refused",
      targetType: "run",
      targetId: runId,
      metadata: { type, state: run.state },
    });
    throw new HttpError(
      409,
      `Command ${type} is not allowed from ${run.state}`,
    );
  }
  ...
  const command = await prisma.$transaction(async (tx) => {
    const created = await tx.runCommand.create({
      data: {
        projectId,
        runId,
        idempotencyKey,
    ...
    await tx.runEvent.create({
      data: eventData(
        ...
        "human",
        actorId,
        { commandId: created.id, command: type },
      ),
    });
    await tx.executionRun.update({
      where: { id: runId },
      data: { state: nextState, lastSequence: sequence },
    });
    await tx.workSession.update({
      where: { id: run.sessionId },
      data: { state: sessionState, version: { increment: 1 } },
    });
    return created;
  });
  await recordAudit(prisma, {
    ...
    targetType: "run",
    targetId: runId,
    metadata: { commandId: command.id, resultState: nextState },
  });
  return command;
}
...
export function buildDeterministicPlan(input: {
  ...
  macroTask: MacroTaskContract;
  graph: TaskGraphContract;
  ...
  const macroTask = parseMacroTask({
    ...
    constraints: [
      "Aucun effet externe depuis le worker simulé",
      "Conserver projectId et sessionId",
    ],
    ...
    acceptanceCriteria: [
      "Les epics couvrent la demande",
      "Chaque ticket possède des critères et une définition de fini",
      "Toutes les tâches respectent leurs dépendances",
      "Le run atteint la revue humaine",
    ],
    riskLevel,
    requiredApprovals: approvalRequirement(riskLevel),
    requiredCapabilities: [
      ...new Set(nodes.map(({ capability }) => capability)),
    ],
...
export function scheduleWaves(
  tasks: readonly Pick<
    RunTask,
    "id" | "taskId" | "dependsOn" | "attempt" | "expectedArtifacts"
  >[],
  capacity: number,
) {
  if (!Number.isInteger(capacity) || capacity < 1 || capacity > 100)
    throw new HttpError(500, "Scheduler capacity is invalid");
  ...
  while (pending.size > 0) {
    const ready = [...pending.values()]
      ...
      .slice(0, capacity);
    if (ready.length === 0)
      throw new HttpError(409, "Task graph is blocked or cyclic");
    waves.push(ready);
...
function commandState(state: string, command: "pause" | "resume" | "cancel") {
  if (command === "pause" && (state === "queued" || state === "running"))
    return "blocked" as const;
  if (command === "resume" && state === "blocked") return "queued" as const;
  if (
    command === "cancel" &&
    ["queued", "running", "blocked", "awaiting_approval"].includes(state)
  )
    return "cancelled" as const;
  return null;
}

function parsePersistedGraph(
  projectId: string,
  sessionId: string,
  macroTaskId: string,
...
function bodyChoice<const T extends readonly string[]>(
  input: Record<string, unknown>,
  key: string,
  allowed: T,
  fallback: T[number],
