...
function route(pathWithSearch: string, onNavigate: (path: string) => void, login: string) {
  ...
  if (path.startsWith('/tickets/')) return <React.Suspense fallback={<StatePanel title="Chargement du ticket…" description="Ouverture du module de détail." />}><LazyTicketDetailPage id={decodeURIComponent(path.split('/')[2])} onNavigate={onNavigate} login={login} /></React.Suspense>;
